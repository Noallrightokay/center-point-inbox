...
export async function POST(req) {
  ...
  for (const { port, secure } of SMTP_PORTS) {
    ...
    try {
      ...
      /* A refusal is an answer: the server exists and said no, so trying the
         other port would only repeat it — and repeating a bad password is how
         accounts get locked. */
      if (/auth|credential|password|535|534/i.test(lastError)) break;
    } finally {
      try { tx.close(); } catch {}
    }
  }
  ...
  return NextResponse.json({
    error: /auth|credential|password|535|534/i.test(lastError)
